...
All file paths are relative to the script location.

Usage:
    Double-click the executable file or run: python financial_report_analyzer.py
"""
...
class FinancialReportAnalyzer:
    ...
    def _load_prompts(self) -> Dict[str, str]:
        ...
        """
        prompt_file = os.path.join(self.script_dir, "prompts.txt")
        logger.info(f"Loading prompts from: {prompt_file}")
        
        try:
            if not os.path.exists(prompt_file):
                raise FileNotFoundError(f"Prompt file 'prompts.txt' not found in the same directory as the script.")
    ...
    def _read_urls_from_file(self) -> List[str]:
        ...
        """
        url_file = os.path.join(self.script_dir, "urls.txt")
        logger.info(f"Reading URLs from: {url_file}")
        
        try:
            if not os.path.exists(url_file):
                raise FileNotFoundError(f"URL file 'urls.txt' not found in the same directory as the script.")
    ...
    def analyze_all_reports(self) -> Dict[str, bool]:
        ...
        """
        output_dir = os.path.join(self.script_dir, "individual_analysis")
...
def main():
    ...
    try:
        ...
        try:
            ...
            show_error(
                "Missing API Key",
                "Please add your OpenAI API key to the .env file as:\nOPENAI_API_KEY=sk-your-actual-key-here"
            )
            return 1
